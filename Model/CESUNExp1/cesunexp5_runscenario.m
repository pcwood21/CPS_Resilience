function  [clientProfits,randLatency,DSim,residualHist,priceHist,failedClients]=cesunexp5_runscenario(runTime,mu_target,sigma_log,failureCount,overrideLatencies)
...
if nargin >= 5 && ~isempty(overrideLatencies)
    randLatency=overrideLatencies;
else
    mu=log(mu_target);
    sigma=exp(sigma_log);
    randLatency=lognrnd(mu,sigma,length(aList),1);
end
