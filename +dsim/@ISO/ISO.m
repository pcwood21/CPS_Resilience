classdef ISO < dsim.Agent
	...
	methods
		function obj=ISO()
            obj.OPT=[];
            if obj.useSimpleFeedback==1
                obj.FeedbackOpt=dsim.FOPTIM();
                obj.FeedbackOpt.lastTarget=63.25;
            else
                obj.OPT=dsim.OPTIM();
            end
		end
        ...
        function [absval,val]=optfun(obj)
            val=sum(obj.clientPower);
            absval=abs(sum(obj.clientPower));
        end
		...
		function X=processPrices(obj,time)
            if obj.useSimpleFeedback == 1
                [~,error]=obj.optfun();
                obj.FeedbackOpt.putObjective(error);
                X=obj.FeedbackOpt.getTarget();
            else
